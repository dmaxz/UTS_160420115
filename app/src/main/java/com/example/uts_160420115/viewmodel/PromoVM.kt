...
class PromoVM(application: Application): AndroidViewModel(application) {
    ...
    fun refresh(restoId: Int){
        ...
        var url = "https://kenhosting.ddns.net/anmp/promo.json"
...
//                Log.e("filtered", restoId.toString())
                var filtered:ArrayList<Promo> = ArrayList<Promo>()
                result.forEach {
                    if (it.restaurant_id == restoId){
                        Log.e("filtered", it.toString())
                        filtered.add(it as Promo)
                    }
                }
                promoLD.value = filtered as ArrayList<Promo>?
